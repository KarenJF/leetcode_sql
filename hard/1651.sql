...
WITH RECURSIVE monthly_tb as (
    select 1 as month
    UNION ALL
    select month + 1 as month from monthly_tb
    where month < 12),
...
ride_summary as (
    ...
    left join (
        select
            ...
            sum(a.ride_distance) as ride_dist,
            sum(a.ride_duration) as ride_dur
        from Rides r
...
summary as (
    select 
        month, 
        IFNULL(ride_dist,0) as ride_dist_1st,
        IFNULL(LEAD(ride_dist,1) over (order by month),0) as ride_dist_2nd,
        IFNULL(LEAD(ride_dist,2) over (order by month),0) as ride_dist_3rd,
        IFNULL(ride_dur,0) as ride_dur_1st,
        IFNULL(LEAD(ride_dur,1) over (order by month),0) as ride_dur_2nd,
        IFNULL(LEAD(ride_dur,2) over (order by month),0) as ride_dur_3rd
    from ride_summary
    order by month
    )
...
select 
    month,
    round(sum(ride_dist_1st+ride_dist_2nd+ride_dist_3rd)/3,2) as average_ride_distance,
    round(sum(ride_dur_1st+ride_dur_2nd+ride_dur_3rd)/3,2) as average_ride_duration
from summary
where month <=10
group by month
...
ride_summary as (
    select 
        m.month,
        IFNULL(tmp.ride_dist,0) as ride_dist,
        IFNULL(tmp.ride_dur,0) as ride_dur
    from monthly_tb m
    left join (
        select
            month(r.requested_at) as month,
...
from (
    select 
        month,
        round(avg(ride_dist) over (order by month rows between 0 preceding and 2 following), 2) as average_ride_distance,
        round(avg(ride_dur) over (order by month rows between 0 preceding and 2 following), 2) as average_ride_duration
    from ride_summary
    order by month) a
where month <=10

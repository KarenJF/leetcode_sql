...
WITH table_a as (
    ...
    table_b as (select count(*) as request_count
    from (select distinct sender_id, send_to_id from FriendRequest) as B)
    
select round(ifnull(accepted_count/request_count,0),2) as accept_rate
from table_a, table_b

-- Solution 2: 
select 
round(ifnull((select count(*) from (select distinct requester_id, accepter_id from RequestAccepted) as A) / 

...
WITH Table_A as (
    ...
    Table_B as (
        select distinct buyer_id_a
        from (
        select 
            a.buyer_id as buyer_id_a, 
            a.product_name as product_name_a,
            b.buyer_id as buyer_id_b,
            b.product_name as product_name_b
        from Table_A a
        Join Table_A b
        on a.buyer_id = b.buyer_id 
        where (a.product_name = 'S8' and b.product_name = 'iPhone')) c)
        
select 
    buyer_id
from Table_A 
where product_name = 'S8' and 
      buyer_id not in (select distinct buyer_id_a from Table_B)

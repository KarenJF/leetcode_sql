...
WITH paid_by as (
    ...
    from Users u
    left join paid_by on u.user_id = paid_by.user
    left join received_by on u.user_id = received_by.user     
    )
    
select 
    user_id,
    user_name,
    credit - paid_amount + received_amount as credit
    (case when (credit - paid_amount + received_amount) < 0 then 'Yes' else 'No' end) as credit_limit_breached
from summary
group by 1,2

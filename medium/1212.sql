...
select 
    t.team_id, 
    t.team_name,
    sum(ifnull(point,0)) as num_points 
from Teams t
left join match_points m
on t.team_id = m.team
group by 
    t.team_id, 
    t.team_name
...
select 
    t.team_id, 
    t.team_name, 
    sum(ifnull((case when t.team_id = host_team and host_goals > guest_goals then 3
          when t.team_id = host_team and host_goals = guest_goals then 1
          when t.team_id = guest_team and guest_goals > host_goals then 3
          when t.team_id = guest_team and guest_goals = host_goals then 1
